...
import {
  fetchHostProperties, fetchHostBookings,
...
// Circle fill colors (saturated, for date cells)
...
// Strip colors (pastel, for range connectors between booking days)
...
export default function AvailabilityPage() {
  ...
  // Range strip info: whether booking extends left/right from this cell
  function getRangeInfo(d: Date): { status: string | null; stripLeft: boolean; stripRight: boolean } {
    ...
    const stripLeft  = !isMonday && !!dateBookingMap.get(prev)?.some(pb => bks.some(b => b.id === pb.id));
    const stripRight = !isSunday && !!dateBookingMap.get(next)?.some(nb => bks.some(b => b.id === nb.id));
  ...
  if (!user) return (
    <div className="min-h-screen bg-gray-50 flex items-center justify-center">
      <p className="text-gray-500 text-sm">로그인이 필요합니다.{" "}
        <Link href="/host" className="text-indigo-600 underline">호스트 로그인</Link>
      </p>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-[#f7f7f7]">
      ...
      <main className="max-w-lg mx-auto px-4 py-5 space-y-3" style={{ paddingBottom: blockMode ? "156px" : "32px" }}>

        {properties.length === 0 ? (
          <div className="text-center py-24 bg-white rounded-3xl">
            <p className="text-5xl mb-4">🏡</p>
            <p className="font-semibold text-gray-700 mb-4">등록된 숙소가 없습니다</p>
            <Link href="/host/new" className="inline-block bg-gray-900 text-white px-6 py-3 rounded-2xl font-semibold text-sm">
              숙소 등록하기
            </Link>
          </div>
        ) : (
